    ...
    private void UpdateVisual() {
        ...
        for (int x = 0; x < grid.GetWidth(); x++) {
            for (int y = 0; y < grid.GetHeight(); y++) {
                ...
                if (pathNode.isWalkable) {
                    if (pathNode.isSand)
                    {
                        MeshUtils.AddToMeshArrays(vertices, uv, triangles, index,
                            grid.GetWorldPosition(x, y) + quadSize * .5f, 0f, quadSize, new Vector2(0.4f, 0.4f), new Vector2(0.45f, 0.45f));
                    }
                    else
                    {
                ...
                {
                    MeshUtils.AddToMeshArrays(vertices, uv, triangles, index,
                            grid.GetWorldPosition(x, y) + quadSize * .5f, 0f, quadSize, Vector2.zero, Vector2.zero);
                }
